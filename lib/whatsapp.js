...
const MEDIA_DOWNLOAD_TIMEOUT_MS = Number(process.env.WHATSAPP_MEDIA_DOWNLOAD_TIMEOUT_MS || 12000);
...
function timeoutSignal(ms) {
  ...
  return AbortSignal.timeout(Math.max(1000, Number(ms) || MEDIA_DOWNLOAD_TIMEOUT_MS));
}
...
async function downloadMedia(mediaId) {
  ...
  const r1 = await fetch(`${WA_BASE}/${mediaId}`, {
    ...
    signal: timeoutSignal(MEDIA_DOWNLOAD_TIMEOUT_MS),
  });
